...
def get_api_key(
    ...
    api_key = db_get_api_key(db, api_key_header)
    if api_key:
        return api_key_header
...
def register_api_key(db: Session, user_id: str) -> str:
    """
    Register a new API key for a user.
    """
    api_key = db_create_api_key(db, user_id)
    return api_key.key
...
def revoke_api_key(db: Session, api_key: str) -> bool:
    ...
    Revoke an API key.
    """
    return db_delete_api_key(db, api_key)
