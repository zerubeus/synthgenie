...
from synthgenie.models.api_key import (
    get_user_api_keys,
    get_api_key_usage,
    get_all_api_key_usage,
)
from synthgenie.schemas.api_key import (
    ApiKeyResponse,
    ApiKeyRequest,
    RevokeRequest,
    ApiKeyUsage,
...
async def get_all_usage_stats(
    ...
    if not admin_api_key or admin_key != admin_api_key:
        raise HTTPException(
            status_code=403, detail="Only admin can view usage statistics"
        )
...
async def get_key_usage_stats(
    ...
    if not usage:
        raise HTTPException(
            status_code=404, detail="API key not found or no usage data"
        )
